...
router.post('/login2', function(req, res, next){
  ...
  .then(datos=>{
    ...
    if (password == concat){
      ...
      res.render('loginfailcliente', {title: 'Login Fail Client'});
    }
  });
});
...
router.post('/payments', async (req, res, next)=>{
  ...
  try{
    const response = await axios.post ('https://fakepayment.onrender.com/payments', payments, {headers:{ Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJuYW1lIjoiSm9obiBEb2UiLCJkYXRlIjoiMjAyNC0wMS0xM1QwMjozODoxNi4yNjFaIiwiaWF0IjoxNzA1MTEzNDk2fQ.GEAoiExqo4goYfmaL7R_Tsm6MzSKqkgCwtW-YKAeY4I'}});
...
//Filtrado de productos por tonos
router.post('/filtrotn', function(req, res, next){
  const {tono} = req.body;  
  console.log(req.body);
  productosModel
    .filtradotn(tono)
...
router.post('/aggprd', function(req, res, next){
  const {nombre, precio, codigo, descripcion, marca, tono, categoria_id} = req.body;
  productosModel
...
router.post('/updateprd/:id', function(req, res, nexte){
  const id= req.params.id;
  const {nombre, precio, codigo, descripcion, marca, tono} = req.body;
  productosModel
